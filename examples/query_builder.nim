...
##
##   nim c --path:src --run examples/query_builder.nim
##
...
proc main() =
  ...
  let table = "example_query_" & intToStr(getTime().toUnix())
  ...
  try:
    ...
    # expects an i64 bound and rejects floating-point values). range_f64 also
    # requires the inclusivity flags (min_inclusive/max_inclusive ->
