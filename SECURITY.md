...
This document describes the security properties of the MongrelDB Nim client
and how to report vulnerabilities.

## Overview

The MongrelDB Nim client is a pure-Nim library (no C ABI bindings, no
external dependencies) that talks to `mongreldb-server` over HTTP using the
standard library `std/httpclient` and `std/json`. The client itself holds
no encryption keys and stores no data at rest; it is a thin request/response
...
The MongrelDB Nim client has no runtime dependencies beyond the Nim
...
- the MongrelDB Nim client version, Nim version, and OS,
